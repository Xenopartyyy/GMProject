...
@section('kontendashboard')
    ...
    <div class="container mx-auto px-4">
        ...
        <form action="/dashboard/brand/{{ $brand->id }}" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-lg shadow-lg space-y-6">
            @method('put')
            @csrf
            ...
            <div>
                <label for="descsingkatbrand" class="block text-lg font-semibold text-gray-700">Deskripsi Singkat Brand</label>
                <input type="text" name="descsingkatbrand"
                    ...
                    value="{{ old('descsingkatbrand', $brand->descsingkatbrand) }}" placeholder="Masukkan Deskirpsi Brand" />
                @error('brand')
                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="deskripsibrand"
                    class="mt-2 w-full rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition duration-200 p-3 @error('brand') @enderror"
                    value="{{ old('deskripsibrand', $brand->deskripsibrand) }}" placeholder="Masukkan Deskirpsi Brand" />
                @error('brand')
                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-end space-x-4 pt-4">
                ...
                <a href="{{ url('/dashboard/brand') }}" class="px-6 py-3 bg-gray-600 text-white font-semibold rounded-lg shadow-md hover:bg-gray-500 transition duration-300">
                    Batal
                </a>
            </div>
        </form>
