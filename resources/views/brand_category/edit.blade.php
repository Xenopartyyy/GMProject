...
@section('kontendashboard')
    ...
    <div class="container mx-auto px-4">
        ...
        <form action="/dashboard/brandcategory/{{ $brand_category->id }}" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-lg shadow-lg space-y-6">
            @method('put')
            @csrf
            ...
            <div>
                ...
                <select name="kategori_id"
                    ...
                    @foreach($kategoris as $kategori)
                        <option value="{{ $kategori->id }}" {{ old('kategori_id', $brand_category->kategori_id) == $kategori->id ? 'selected' : '' }}>
                            {{ $kategori->nmkategori }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="brands_id"
                    ...
                    @foreach($brands as $brand)
                        <option value="{{ $brand->id }}" {{ old('brands_id', $brand_category->brands_id) == $brand->id ? 'selected' : '' }}>
                            {{ $brand->namabrand }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex items-center justify-end space-x-4 pt-4">
                ...
                <a href="{{ url('/dashboard/brandcategory') }}" class="px-6 py-3 bg-gray-600 text-white font-semibold rounded-lg shadow-md hover:bg-gray-500 transition duration-300">
                    Batal
                </a>
            </div>
        </form>
