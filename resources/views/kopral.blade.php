...
@section('konten')
    ...
    <div class="flex flex-wrap md:flex-nowrap">  
                ...
                <div id="geseroverflowy" class="mx-5 space-y-5 my-5 overflow-hidden max-h-[calc(100vh-200px)] relative">  
                    ...
                    @foreach ($brand_category as $bc)  
                        @if (Str::contains($bc->brand->namabrand, 'Kopral'))  
                            <div class="relative w-30 h-30 sm:w-30 sm:h-30 flex flex-col items-center justify-center animate-slide-down">  
                                <div class="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center flex-col">  
                                    <h3 class="text-2xl font-semibold text-white mb-2 text-center">{{ $bc->kategori->nmkategori }}</h3>  
                                    <p class="text-white text-opacity-90 text-center">{{ $bc->descatbrands }}</p>  
                                </div>  
                                <img src="{{ $bc['fotocatbrands'] }}" alt="{{ $bc->kategori->nmkategori }}" class="w-full h-full object-cover" />  
                            </div>  
                        @endif  
                    @endforeach  
        ...
        <section id="katalog-produk" class="w-full md:w-4/5 bg-white relative">  
            ...
            <div class="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 py-12 px-24 text-white">  
                @foreach ($produk->where('brand.namabrand', 'Kopral')->take(3) as $p)  
                    <div class="bg-white px-10 shadow-lg rounded-2xl overflow-hidden transform transition-transform hover:scale-105 hover:shadow-2xl">  
                        @php  
