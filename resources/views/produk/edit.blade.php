...
@section('kontendashboard')
    ...
    <div class="container mx-auto px-4">
        ...
        <form action="{{ url('/dashboard/produk/' . $produk->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <input type="text" name="link_shopee"
                    ...
                    value="{{ old('link_shopee', $produk->link_shopee) }}"
            ...
            <div>
                ...
                <input type="text" name="link_tokped"
                    ...
                    value="{{ old('link_tokped', $produk->link_tokped) }}"
            ...
            <div>
                ...
                <input type="text" name="link_ttshop"
                    ...
                    value="{{ old('link_ttshop', $produk->link_ttshop) }}"
                    placeholder="Masukkan Link Barang di TiktokShop ( kosongkan jika tidak ada )" />
                @error('link_ttshop')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
                ...
                @if (!empty($produk->fotobrg) && is_array($produk->fotobrg))
                    @foreach ($produk->fotobrg as $index => $foto)
                        <div class="flex items-center space-x-4">
                            <img src="{{ $foto }}" alt="Gambar {{ $index + 1 }}"
                                class="w-24 h-24 object-cover border rounded-lg">
                            <input type="hidden" name="existing_images[]" value="{{ $foto }}">
                            <button type="button"
                                class="remove-current-image bg-red-500 text-white px-3 py-2 rounded-md hover:bg-red-400 transition"
                                data-foto="{{ $foto }}">
                                Hapus
                            </button>
                        </div>
            ...
            <!-- Input tersembunyi untuk gambar yang dihapus -->
            ...
            <div id="image-upload-container" class="space-y-4 mt-4">
                <div class="image-upload-item flex items-center space-x-4">
                    <input type="file" name="fotobrg[]"
                        class="w-full rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition duration-200 p-3">
                    <button type="button"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Jika deleted_images tidak ada, buat array kosong
            let deletedImages = deletedImagesInput.value ? deletedImagesInput.value.split(',') : [];

            // Fungsi untuk menambahkan gambar yang dihapus ke dalam array
            ...
            currentImagesContainer.addEventListener('click', (e) => {
                if (e.target && e.target.classList.contains('remove-current-image')) {
                    const image = e.target.getAttribute('data-foto');
                    if (!deletedImages.includes(image)) {
                        deletedImages.push(image); // Tambahkan gambar yang dihapus ke dalam array
                    }
                    updateDeletedImages(); // Update input tersembunyi dengan gambar yang dihapus

                    const parentDiv = e.target.closest('div');
                    parentDiv.remove(); // Hapus gambar dari DOM
                }
            });
            ...
            <button type="button" class="remove-image bg-red-500 text-white px-3 py-2 rounded-md hover:bg-red-400 transition">Hapus</button>
        `;
                document.getElementById('image-upload-container').appendChild(newImageField);

                // Attach event to remove button of new image
                const removeButton = newImageField.querySelector('.remove-image');
                removeButton.addEventListener('click', () => {
                    document.getElementById('image-upload-container').removeChild(newImageField);
                });
            });
        });
    </script>
