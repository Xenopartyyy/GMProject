...
<div class="container mx-auto px-4">
  <h1 class="text-center text-2xl font-semibold my-5">Data Slider</h1>
  <div class="flex justify-end mb-4">
    <a href="{{ url('dashboard/slider/create') }}" class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded">Tambah Slider</a>
  </div>
  <div class="overflow-x-auto mt-6">
    <table id="sliderTable" class="min-w-full bg-white border border-gray-300 ">
      ...
      <tbody>
        @foreach ($slider as $index => $sldr)
          <tr class="text-center hover:bg-gray-100 border-b border-gray-300 justify-center center">
            <td class="py-2 px-4 border border-gray-300 text-center">{{ $index + 1 }}</td>
            <td class="py-2 px-4 border border-gray-300">
              <div class="flex justify-center space-x-2">
                <a href="{{ url('dashboard/slider/' . $sldr->id . '/edit') }}" class="bg-yellow-400 hover:bg-yellow-500 text-white font-semibold py-1 px-2 rounded" title="Edit">
                  <i class="fa-regular fa-pen-to-square"></i>
                </a>
                <form action="{{ url('dashboard/slider/' . $sldr->id) }}" method="POST" class="inline-block">
                  @csrf
                  @method('DELETE')
            ...
            <td class="py-2 px-4 border border-gray-300">{{ $sldr->nama }}</td>
            <td class="py-2 px-4 border border-gray-300">
              <img src="{{ asset('storage/banner/' . $sldr->banner) }}" class="w-12 h-12 object-cover rounded" alt="Banner">
            </td>
          </tr>
        @endforeach
      </tbody>
