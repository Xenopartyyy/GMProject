...
<div class="container mx-auto px-4">
  <h1 class="text-center text-2xl font-semibold my-5">Data Testimoni</h1>
  <div class="flex justify-end mb-4">
    <a href="{{ url('dashboard/testimoni/create') }}" class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded">Tambah Testimoni</a>
  </div>
    ...
    <table id="testimoniTable" class="min-w-full bg-white border border-gray-300">
      <thead class="border-b border-gray-300">
        <tr>
          ...
          <th class="py-2 px-4 border border-gray-300">Text Review</th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr class="text-center hover:bg-gray-100 border-b border-gray-300">
          ...
          <td class="py-2 px-4 border border-gray-300">
            <div class="flex justify-center space-x-2">
              <a href="{{ url('dashboard/testimoni/' . $testi->id . '/edit') }}" class="bg-yellow-400 hover:bg-yellow-500 text-white font-semibold py-1 px-2 rounded" title="Edit">
                <i class="fa-regular fa-pen-to-square"></i>
              </a>
              <form action="{{ url('dashboard/testimoni/' . $testi->id) }}" method="POST" class="inline-block">
                @csrf
                @method('DELETE')
          ...
          <td class="py-2 px-4 border border-gray-300">{{ $testi->namacust }}</td>
          <td class="py-2 px-4 border border-gray-300">{{ $testi->bintang }}</td>
          <td class="py-2 px-4 border border-gray-300">{{ $testi->review }}</td>
                  </tr>
        @endforeach
      </tbody>
    </table>
